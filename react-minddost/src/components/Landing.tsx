...
import { XMarkIcon, MicrophoneIcon, ChatBubbleLeftRightIcon } from '@heroicons/react/24/outline';
import axios from 'axios';
import '../styles/fonts.css'; // Make sure to create this file
...
export default function Landing() {
    ...
    if (message && !isTyping) {
      ...
      const typeInterval = setInterval(() => {
        if (index < message.length-1) {
          ...
          setDisplayMessage((displayMessage) => displayMessage + message[index]);
          index++;
        } else {
  ...
  const handleEndChat = async () => {
    try {
      await axios.post('http://localhost:5000/api/sessions/end', {
  ...
  const handleVoiceInput = () => {
    ...
    recognition.onresult = async (event) => {
      ...
      try {
        const response = await axios.post('http://localhost:5000/api/sessions/chat', {
          sessionId: localStorage.getItem('currentSessionId'),
          message: transcript
        }, {
          headers: {
      ...
      <main className="flex-1 flex flex-col items-center justify-center">
        <div className="absolute top-6 right-6 flex items-center space-x-2">
          ...
          <Switch
            ...
            className={`${genZMode ? 'bg-gray-800' : 'bg-gray-300'} relative inline-flex h-6 w-11 items-center rounded-full`}
          >
            <span className="sr-only">Toggle Gen Z Mode</span>
            <span
              className={`${genZMode ? 'translate-x-6' : 'translate-x-1'} inline-block h-4 w-4 transform bg-white rounded-full transition`}
            />
          </Switch>
        </div>
        <div className="flex flex-col items-center">
          <p className="mb-6 text-lg bg-white text-gray-900 font-switzer font-semibold">burning the midnight oil!</p>
          <div className="w-60 h-60 bg-yellow-400 rounded-full transform transition-transform duration-100 animate-breathe"></div>
        </div>
        <div className="mt-6 flex gap-2">
          <button
            className={`px-4 py-2 text-sm rounded font-satoshi ${sessionMode === 'classic' ? 'bg-gray-900 text-white' : 'bg-gray-300 text-gray-700'}`}
            onClick={() => setSessionMode('classic')}
          >
            classic
          </button>
          <button
            className={`px-4 py-2 text-sm rounded font-satoshi ${sessionMode === 'guided' ? 'bg-gray-900 text-white' : 'bg-gray-300 text-gray-700'}`}
        ...
        {sessionStarted ? (
        <div className="mt-6 min-h-[100px] w-full max-w-md bg-white p-4 flex flex-col items-center justify-between gap-4">
          <p className="text-gray-900 whitespace-pre-wrap bg-gray-100 rounded-lg p-4 font-satoshi">
            {displayMessage}
            {isTyping && <span className="animate-pulse">|</span>}
          </p>
          
          {/* Control buttons */}
          <div className={`w-full bottom-4 flex flex-row justify-between items-center ${isTyping ? 'disabled' : 'flex'}`}>
            <button
              onClick={handleEndChat}
              className={`p-4 rounded-full bg-red-100 text-red-600 hover:bg-red-200 transition-colors ${isTyping ? 'disabled' : 'flex'}`}
              title="End Chat"
            >
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-switzer font-semibold">Chat Format</h3>
                <button
              ...
              <div className="space-y-4">
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-blue-100 flex items-center justify-center">
                    <span className="text-blue-600 font-switzer font-semibold">Y</span>
                  </div>
                  <div className="flex-1 bg-blue-50 rounded-lg p-3">
                    <p className="text-gray-900 font-satoshi">Your message appears here</p>
                  </div>
                </div>
                <div className="flex items-start space-x-3">
                  <div className="flex-shrink-0 w-8 h-8 rounded-full bg-yellow-100 flex items-center justify-center">
                    <span className="text-yellow-600 font-switzer font-semibold">A</span>
                  </div>
                  <div className="flex-1 bg-yellow-50 rounded-lg p-3">
                    <p className="text-gray-900 font-satoshi">AI response appears here</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        {sessionStarted ? (
          ...
          <button onClick={handleSessionStart} className="mt-4 bg-yellow-400 hover:bg-yellow-500 text-black font-switzer font-semibold py-2 px-6 rounded">
            begin session
          </button>
        )}
      </main>
